...
class litecode():
    def run(file):
        global args, variables, ifkosul, readyrunner, veri, temp_file, app_args, komut, current_line, dongu
        directory = os.getcwd()
        ...
        try:
            ...
            exit(f"Hata({komut}): dosya bulunamadı.")
        try:
        ...
        while current_line < len(lines):
            ...
            if satir.startswith("   ") or satir.startswith(" ") or re.match(r"^\s", satir):
                try:
                    if not eval(ifkosul):
                        current_line += 1
                        continue
...
if len(sys.argv) > 1:
    ...
    dongu = 0
    hata = 0
    baslangic_zamani = time.perf_counter() # ne kadar hızlı olduğunu ölçücez ;)
    litecode.run(sys.argv[1])
    bitis_zamani = time.perf_counter()
    gecen_sure = bitis_zamani - baslangic_zamani
    print(f"{dongu} satır kod {gecen_sure:.4f} saniyede işlendi.")
else:
    print(f"Hata({komut}): Bir dosya adı belirtmelisiniz.") #litecode.run("litecode.lc")
exit()